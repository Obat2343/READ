...
for task_index, task_name in enumerate(args.tasks):
    if task_index == 0:
        ...
        model_config_path = os.path.join(model_path[:model_path.find("/model")], "Train_CVAE.yaml")
    else:
    ...
    if model_name == "resnet18":
        model = CVAE_resnet(input_keys, input_dims, 101, img_size=256, predictor='transformer', model_name='resnet18', latent_dim=cfg.VAE.LATENT_DIM)
    elif model_name == "resnet18_mlp":
        model = CVAE_resnet(input_keys, input_dims, 101, img_size=256, predictor='mlp', model_name='resnet18', latent_dim=cfg.VAE.LATENT_DIM)
    else:
        model = CVAE(input_keys, input_dims, 101, 
                dims=conv_dims, enc_depths=enc_depths, enc_layers=enc_layers, dec_depths=dec_depths, dec_layers=dec_layers, 
                latent_dim=cfg.VAE.LATENT_DIM, intrinsic=train_dataset.info_dict["data_list"][0]["camera_intrinsic"]).to(device)
    ...
    for index in range(max_index):
        ...
        with torch.no_grad():
            pred_action = model.sample(image, device)
