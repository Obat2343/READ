...
from pycode.DF import sampling
...
for task_index, task_name in enumerate(args.tasks):
    ...
    if model_name == "Convnext-UNet":
        model = SPE_Continuous_Diffusion_CG(input_keys, input_dims,
    ...
    if sde_name == 'vpsde':
        print(f"beta_min: {cfg.SDE.VPSDES.BETA_MIN}")
    ...
    sampling_fn = sampling.get_sampling_fn(cfg, sde, shape_dict, inverse_scaler, sampling_eps, guidance=True)
        ...
        for index in range(max_index):
            ...
            if inference_method == "random":
                ...
                if "wo_modification" in inference_method:
                    ...
                    pred_action, history, n = sampling_fn(model, x=retrieved_query, condition=image, history=True, N=cfg.SDE.N, CG=True)
            else:
            ...
            #         history_list.append(get_pos(history["x_mean"][his_index*100], val_dataset.info_dict["data_list"][0]["camera_intrinsic"]))
