    ...
    for inference_method in inference_method_list:
        ...
        for index in range(max_index):
            ...
            if inference_method == "random":
                pred_action, n = sampling_fn(model, condition=image, latent=True)
                ...
                if "wo_modification" in inference_method:
                    ...
                    pred_action, n = sampling_fn(model, x=retrieved_query, condition=image, latent=False, N=cfg.SDE.N)
            ...
            if "retrieve" in inference_method:
                ...
                for key in near_queries.keys():
                    for rank_index in range(8):
                        near_query_list[rank_index][key] = near_queries[key][:,rank_index]
