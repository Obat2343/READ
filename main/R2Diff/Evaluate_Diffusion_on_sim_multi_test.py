    ...
    for inference_method in inference_method_list:
        ...
        if args.result_dirname != "":
            ...
            if "retrieve" in inference_method:
                result_dir_name = f"{inference_method}_top{cfg.RETRIEVAL.RANK}_step{cfg.DIFFUSION.STEP_EVAL}_multitest"
            else:
        ...
        for index in range(max_index):
            ...
            if "retrieve" in inference_method:
                ...
                for rank in range(5):
                    ...
                    for key in near_queries.keys():
                        retrieved_query[key] = near_queries[key][:,rank].to(device)
                
    
                    pred_action = model.reconstruct(image, retrieved_query, reconst_steps)[0]
            ...
            vis_img = visualize_multi_query_pos(image, pred_action_list, val_dataset.info_dict["data_list"][0]["camera_intrinsic"], rot_mode=rot_mode)
            ...
            vis_img.save(os.path.join(result_motion_path, f"pred_motion_lists_{str(index).zfill(5)}.png"))
            ...
            for pred_action in pred_action_list:
                success = False
                total_reward = 0
                try_iter = 0
                ...
                # save images
                if success:
                    image_seq_dir = os.path.join(result_images_path, f"{str(index).zfill(5)}_rank{rank}_success")
                else:
                    image_seq_dir = os.path.join(result_images_path, f"{str(index).zfill(5)}_rank{rank}_fail")
                ...
                make_video(image_list, os.path.join(result_video_path, f"{str(index).zfill(5)}_rank{rank}.mp4"), (256,256), success)
...
for task_index, task_name in enumerate(args.tasks):
    ...
    for inference_method in inference_method_list:
        ...
        if args.result_dirname != "":
            ...
            if "retrieve" in inference_method:
                result_dir_name = f"{inference_method}_top{cfg.RETRIEVAL.RANK}_step{cfg.DIFFUSION.STEP_EVAL}"
            else:
                result_dir_name = inference_method
