...
parser.add_argument('--config_path', type=str, default="../config/user/Test_config.yaml")
...
for task_index, task_name in enumerate(args.tasks):
    ...
    val_dataset = RLBench_DMOEBM(mode, cfg, save_dataset=False, num_frame=frame, rot_mode=rot_mode, aug_type=args.augment)
    ...
    for inference_method in inference_method_list:
        ...
        for index in range(max_index):
            ...
            if inference_method == "random":
                ...
                pred_action = pred_action_list[0]
                pred_action = get_pos(pred_action, val_dataset.info_dict["data_list"][0]["camera_intrinsic"])
            elif inference_method == "reconstruct_wo_forward":
                reconst_steps = cfg.DIFFUSION.STEP_EVAL
                pred_action_list = model.reconstruct(image, h_query, reconst_steps, forward_diff=False) #TODO modify steps - 1 to steps.
                pred_action = pred_action_list[0]
                pred_action = get_pos(pred_action, val_dataset.info_dict["data_list"][0]["camera_intrinsic"])
