...
from pycode.R2Diff.diffusion import Denoising_Diffusion
    ...
    for inference_method in inference_method_list:
        ...
        if args.result_dirname != "":
            ...
            if "retrieve" in inference_method:
                result_dir_name = f"DMO_{inference_method}"
            else:
        ...
        for index in range(max_index):
            ...
            if inference_method == "random":
                ...
                for key in near_queries.keys():
                    retrieved_query[key] = near_queries[key][:,cfg.RETRIEVAL.RANK-1:cfg.RETRIEVAL.RANK].to(device)
                
                with torch.no_grad():
                    pred_action, _ = DMO_model(image, retrieved_query)
                
                for key in retrieved_query.keys():
                    if key in pred_action.keys():
                        pred_action[key] = pred_action[key][:,0]
                    retrieved_query[key] = retrieved_query[key][:,0]
