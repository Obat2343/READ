...
parser.add_argument('--config_file', type=str, default='../config/Train_Triplet.yaml', metavar='FILE', help='path to config file')
...
if args.name == "":
    dir_name = f"Retrieval_rank_{cfg.RETRIEVAL.RANK}_frame_{args.frame}"
else:
    dir_name = args.name
...
if not args.debug:
    wandb.login()
    run = wandb.init(project='RLBench-Retrieval', entity='tendon', group=cfg.DATASET.RLBENCH.TASK_NAME,
...
Train_loss = losses.get_triplet_loss_fn(train=True)
Val_loss = losses.get_triplet_loss_fn(train=False)
...
for epoch in range(10000000):
    for data in train_dataloader:
        ...
        for key in query.keys():
            ...
            neg_query[key] = neg_query[key].to(device)
        
        # optimization
        optimizer.zero_grad()
        loss, loss_dict = Train_loss(model, image, query, pos_image, pos_query, neg_image, neg_query)
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        if iteration % eval_iter == 0:
            for data in val_dataloader:
                image, query, _, _, _, neg_query = data
            ...
            retrieved_image, retrieved_query = train_dataset.retrieve_k_sample(query, k=index)
            ...
            with torch.no_grad():
                loss, loss_dict = Val_loss(model, image, query, retrieved_image, retrieved_query, neg_image, neg_query)
