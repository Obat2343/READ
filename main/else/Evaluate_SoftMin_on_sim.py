...
from pycode.misc import Direct_Retrieval, Image_Based_Retrieval_SPE, BYOL_Retrieval, CLIP_Retrieval, MSE_Based_Retrieval
    ...
    for inference_method in inference_method_list:
        ...
        if args.result_dirname != "":
            ...
            if "retrieve" in inference_method:
                result_dir_name = f"{inference_method}_SoftMin_top_{cfg.RETRIEVAL.RANK}"
            else:
                raise ValueError("Invalid for SoftMin-predictor")
        ...
        for index in range(max_index):
            ...
            if "retrieve" in inference_method:
                ...
                if inference_method == "retrieve_from_motion":
                    ...
                    near_queries, nears, dists = Retriever.retrieve_k_sample(image, k=cfg.RETRIEVAL.RANK)
                ...
                dists = torch.nan_to_num(dists, nan=0.0, posinf=1e+6)
                weight = softmin(dists)
                B,_ = weight.shape

                # retrieved_queries = convert_rotation_6d_to_matrix([copy.deepcopy(near_queries)])[0]
                retrieved_queries = near_queries

                retrieved_pos = retrieved_queries["pos"]
                retrieved_rot = retrieved_queries["rotation"]
                retrieved_grasp = retrieved_queries["grasp_state"]

                ave_pos = torch.sum(repeat(weight, "B K -> B K S D",S=101, D=3) * retrieved_pos, dim=1)
                ave_rot = torch.sum(repeat(weight, "B K -> B K S D",S=101, D=6) * retrieved_rot, dim=1)
                ave_grasp = torch.sum(repeat(weight, "B K -> B K S D",S=101, D=1) * retrieved_grasp, dim=1)

                pred_action = {"pos": ave_pos, "rotation": ave_rot, "grasp_state":ave_grasp}
            else:
