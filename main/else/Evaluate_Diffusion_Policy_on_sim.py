...
from pycode.misc import load_checkpoint, convert_rotation_6d_to_matrix, visualize_inf_query, get_pos, visualize_multi_query_pos, Concat_query_keys
...
for task_index, task_name in enumerate(args.tasks):
    if task_index == 0:
        ...
        model_config_path = os.path.join(model_path[:model_path.find("/model")], "RLBench_DiffusionPolicy.yaml")
    else:
    ...
    vision_encoder.conv1 = torch.nn.Conv2d(4, 64, kernel_size=(7,7), stride=(2,2), padding=(3,3), bias=False)
    ...
    print(nets)
    print("loading model")
    nets, _, _, _, _ = load_checkpoint(nets, model_path)
    nets.to(device)
    nets.eval()
    ...
    for inference_method in inference_method_list:
        ...
        for index in range(max_index):
            ...
            if inference_method == "random":
                with torch.no_grad():
                    action = converter.concat_query(h_query)        
                    ...
                    for k in noise_scheduler.timesteps:    
                        noise_pred = nets["noise_pred_net"](
                            naction, k, global_cond=obs_cond)
                    ...
                    pred_action = converter.split_query(naction)
                    for key in pred_action.keys():
                        pred_action[key] = torch.cat([h_query[key][:,0:1], pred_action[key]], 1)
