...
class SDE(abc.ABC):
    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, t, condition=None, retrieved=None):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                drift_dict, diffusion = sde_fn(x, t)
                score_dict = score_fn(x, t, condition, retrieved=retrieved)
            ...
            def discretize(self, x, t, condition=None, retrieved=None):
...
class IRSDE(SDE):
    '''
    This code is based on https://github.com/Algolzw/image-restoration-sde/blob/main/codes/utils/sde_utils.py
    '''
    def __init__(self, keys, lamda, schedule='cosine', N=100, eps=0.01, theta_1=2.0):
        super().__init__(N)
        self.lamda = lamda
        self.N = N
        self._initialize(lamda, schedule, eps, theta_1)

    def _initialize(self, lamda, schedule, eps=0.01, theta_1=2.0):

        def constant_theta_schedule(const=100.0):
            """
            constant schedule
            """
            ...
            def theta(timesteps):
                return torch.ones_like(timesteps, dtype=torch.float32) * const

            def theta_bar(timesteps):
                return const * torch.ones_like(timesteps, dtype=torch.float32) * timesteps

            return theta, theta_bar

        def linear_theta_schedule(theta_0=0.01, theta_1=2.0):
            """
            linear schedule
            """
            print('linear schedule')
            def theta(timesteps):
                return theta_0 + timesteps * (theta_1 - theta_0)

            def theta_bar(timesteps):
                return 0.5 * (theta_1 - theta_0) * timesteps ** 2 + (theta_0 * timesteps) 
        ...
        def get_sigmas(t, theta):
            return torch.sqrt((lamda**2) * 2 * theta(t))
            
        if schedule == 'cosine':
            theta, theta_bar = cosine_theta_schedule()
        elif schedule == 'linear':
            theta, theta_bar = linear_theta_schedule(lamda, theta_1=theta_1)
        elif schedule == 'constant':
            theta, theta_bar = constant_theta_schedule()
        else:
            print('Not implemented such schedule yet!!!')

        self.theta = theta
        self.theta_bar = theta_bar
        self.sigma = functools.partial(get_sigmas, theta=theta)
    ...
    def sde(self, x, mu, t):
        ...
        sigma = self.sigma(t)
        for key in x.keys():
            drift_dict[key] = theta * (mu[key] - x[key])
        diffusion = sigma.to(t.device)

        return drift_dict, diffusion

    def marginal_prob(self, x, mu, t):
        mean = {}
        for key in x.keys():
            if x[key].dim() == 2:
                mean[key] = mu[key] + (x[key] - mu[key]) * torch.exp(-self.theta_bar(t)[:, None])
            elif x[key].dim() == 3:
                mean[key] = mu[key] + (x[key] - mu[key]) * torch.exp(-self.theta_bar(t)[:, None, None])

        std = torch.sqrt(self.lamda ** 2 * (1 - torch.exp(-2 * self.theta_bar(t))))
        return mean, std
    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, t, condition=None, retrieved=None):
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                drift_dict, diffusion = sde_fn(x, retrieved, t)
