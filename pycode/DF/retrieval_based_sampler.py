...
def get_sampling_fn(cfg, sde, shape, noise_sampler, inverse_scaler, eps, device="cuda", energy=False):
...
class Predictor(abc.ABC):
    ...
    def __init__(self, sde, score_fn, noise_sampler=None, probability_flow=False):
        super().__init__()
        self.sde = sde
...
class AncestralSamplingPredictor(Predictor):
    ...
    def vpsde_update_fn(self, x, t, condition=None):
        sde = self.sde
        timestep = (t * (sde.N - 1) / sde.T).long()
...
class ColdDiffusionPredictor(Predictor):
    def __init__(self, sde, model_fn, model, probability_flow=False):
        super().__init__(sde, None, probability_flow)
    ...
    def update_fn(self, x, t, condition=None):
        ...
        latent = self.model.latent_AutoEncoder.encode(x)
        x_mean = self.model_fn(latent, t, condition)
        ...
        for key in latent.keys():
            perturbed_latent[key] = mean[key] + std[:, None] * z
...
def shared_predictor_update_fn(x, t, sde, model, predictor, probability_flow, continuous, energy, condition=None):
    ...
    if predictor is None:
        ...
        if probability_flow == True:
            raise NotImplementedError()
        predictor_obj = predictor(sde, model_fn, model, False)
    else:
        predictor_obj = predictor(sde, score_fn, probability_flow)
    return predictor_obj.update_fn(x, t, condition)


def shared_corrector_update_fn(x, t, sde, model, corrector, continuous, energy, snr, n_steps, condition=None):
...
def get_pc_sampler(sde, shape_dict, predictor, corrector, inverse_scaler, snr,
                    n_steps=1, probability_flow=False, continuous=False, energy=False,
    ...
    def pc_sampler(model, x=None, condition=None, start_time_ratio=None, N=None, history=False, latent=False):
        ...
        with torch.no_grad():
            ...
            if latent:
                x = model.latent_AutoEncoder.decode(x)
            ...
            for i in range(N):
                ...
                x, x_mean = predictor_update_fn(x, vec_t, model=model, condition=condition)
                if history:
...
def get_ode_sampler(sde, shape_dict, inverse_scaler,
    ...
    def denoise_update_fn(model, x, condition=None):
        if type(condition) == type(None):
        ...
        _, x = predictor_obj.update_fn(x, vec_eps, condition=condition)
        return x

    def drift_fn(model, x, t, fixed_condition=False):
    ...
    def ode_sampler(model, x=None, start_time_ratio=None, N=None, condition=None):
        ...
        with torch.no_grad():
            ...
            def ode_func(t, x):
                ...
                if type(condition) == type(None):
                    ...
                    drift = drift_fn(model, x, vec_t, fixed_condition=True)
