...
class Resnet_Like_Encoder(nn.Module):
    ...
    def __init__(self, image_size, in_chans=3, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768], layers=['conv','conv','conv','conv'], drop_path_rate=0., 
                    layer_scale_init_value=1e-6, activation='gelu', norm='layer', stem_depth=0):
        super().__init__()
        ...
        for i in range(len(depths)):
            stage = nn.ModuleList(
                ...
                layer_scale_init_value) for j in range(depths[i])]
            )
            self.stages.append(stage)
            cur += depths[i]
    
    @staticmethod
    def build_block(image_size, dim, layer_name, activation, norm, drop_path_rate, layer_scale_init_value):
        if layer_name == 'conv':
...
class Resnet_Like_Decoder(nn.Module):
    ...
    def __init__(self, image_size, depths=[3, 3, 3], enc_dims=[96, 192, 384, 768], layers=['conv','conv','conv'], drop_path_rate=0., 
                    layer_scale_init_value=1e-6, activation='gelu', norm='layer', emb_dim=0):
        super().__init__()

        # difine downsample layer
        if emb_dim == 0:
            emb_dim = enc_dims[0] // 2

        self.last_upsample = nn.Sequential(
            nn.ConvTranspose2d(enc_dims[0], emb_dim, kernel_size=4, stride=4),
        )
        image_size = image_size // 4
        ...
        for i in range(len(depths)):
            ...
            for j in range(depths[i]):
                stage.append(self.build_block(image_size // (2**i), enc_dims[i], layers[i], activation, norm, dp_rates[cur + j], layer_scale_init_value))
            self.stages.append(stage)
            cur += depths[i]
